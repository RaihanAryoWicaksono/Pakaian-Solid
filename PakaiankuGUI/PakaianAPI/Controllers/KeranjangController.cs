...
namespace PakaianApi.Controllers
{
    ...
    public class KeranjangController : ControllerBase
    {
        ...
        // Helper untuk mendapatkan keranjang user, buat atau ambil yang ada
        ...
        // Helper untuk memetakan KeranjangItem entity ke KeranjangItemDto
        ...
        [ProducesResponseType(typeof(ErrorResponse), 400)]
        public async Task<IActionResult> Checkout([FromBody] CheckoutDto checkoutDto, [FromQuery] int userId) // userId sebagai parameter
        {
            if (userId == 0) return BadRequest("User ID diperlukan untuk checkout.");

            var keranjangDb = await GetOrCreateUserCartAsync(userId);
            if (!keranjangDb.Items.Any())
            {
                return BadRequest(new ErrorResponse { Status = 400, Message = "Keranjang kosong. Tidak ada yang bisa di-checkout." });
            }

            foreach (var cartItem in keranjangDb.Items)
            {
                var pakaian = await _context.Pakaian.FirstOrDefaultAsync(p => p.Kode == cartItem.PakaianKode);
                if (pakaian == null || pakaian.Stok < cartItem.Quantity || pakaian.Status != StatusPakaian.DalamKeranjang)
                {
                    return BadRequest(new ErrorResponse
                    {
                        Status = 400,
                        Message = $"Pakaian '{pakaian?.Nama ?? cartItem.PakaianKode}' tidak dapat di-checkout. Stok tidak cukup ({pakaian?.Stok ?? 0} tersedia, {cartItem.Quantity} dibutuhkan) atau status berubah ({pakaian?.Status})."
                    });
                }
            }
